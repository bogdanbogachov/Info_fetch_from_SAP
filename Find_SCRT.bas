...
If WorksheetExists(Check & "_In_SCRT") Then
...
If Trim(FindString) <> "" Then
    ...
    Else
        ...
        With OpenBook.Sheets(1)
            Set aCell = .Range("A5:Z5").Find(What:="part was", LookIn:=xlValues, lookat:=xlWhole, _
                        MatchCase:=False, SearchFormat:=False)
            If Not aCell Is Nothing Then
                col = aCell.Column
                colName = Split(.Cells(, col).Address, "$")(1)
            Else
                MsgBox ("Column PART WAS not found")
            End If
        End With
